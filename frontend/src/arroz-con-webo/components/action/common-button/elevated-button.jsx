 ...
 */
export default function ElevatedButton({className, role = "neutral", pill, ripple, onClick, children}) {
    if(role && !valid_role(role))
        throw new InvalidRoleError({"code": "Invalid props.role value.", "value": role});

    const classes = useStyles({role});
    return (
        <Button className={`${classes['arroz-elevated-button']} ${className?? ""}`} pill={pill} ripple={ripple} onClick={onClick}>
            {children}
        </Button>
    );
}